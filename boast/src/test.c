...
#include <math.h>
#include <time.h>
//#include "d_s1s0_1d_sym8.c"
//#include "d_s0s1_1d_sym8.c"
...
*/
	...
	//d_s0s1_1d_sym8
	int i;
	Matrice m1[NBTESTS]; // matrices de départ de l'espace S1
	Matrice m2[NBTESTS]; // matrices du milieu de l'espace S0
	Matrice m3[NBTESTS]; // matrices d'après IMD de l'espace réel
	Matrice tmp[NBTESTS];
	Matrice res[NBTESTS];
	Matrice tmp2[NBTESTS];
	int dim[3] = {31, 48, 28};		
	#pragma omp task{
	for(i=0; i<NBTESTS; i++){
		...
		initMatricePeriodic(&tmp2[i], 3, dim);
		setMatrice(&m1[i]);
		//printMatrice(m1);
		...
		d_s0s0_1d_sym8_md(m2[i].dimension, 0, m2[i].tailles_s0, PERIODIC, m2[i].tab, res[i].tab, 1.0, 0.0, 0.0);
		d_s0s0_1d_sym8_md(m2[i].dimension, 1, m2[i].tailles_s0, PERIODIC, res[i].tab, tmp[i].tab, 1.0, 0.0, 0.0);
		d_s0s0_1d_sym8_md(m2[i].dimension, 2, m2[i].tailles_s0, PERIODIC, tmp[i].tab, res[i].tab, 1.0, 0.0, 0.0);

		
		/*réel vers peudo réel IMD */
		d_s0s0_1d_sym8_imd(res[i].dimension, 0, res[i].tailles_s0, PERIODIC, res[i].tab, tmp2[i].tab, 1.0, 0.0, 0.0);
		d_s0s0_1d_sym8_imd(res[i].dimension, 1, res[i].tailles_s0, PERIODIC, tmp2[i].tab, tmp[i].tab, 1.0, 0.0, 0.0);
		d_s0s0_1d_sym8_imd(res[i].dimension, 2, res[i].tailles_s0, PERIODIC, tmp[i].tab, m2[i].tab, 1.0, 0.0, 0.0);
		//printDiffMatrice(&m1,&m2[i]);
		

		

		/* Réel vers Ondelettes */
		/*d_s0s1_1d_sym8(m2[i].dimension,0,m1.tailles_s1,PERIODIC,m2[i].tab,res.tab,1.0,0.0);
		d_s0s1_1d_sym8(tmp.dimension,1,m2[i].tailles_s1,PERIODIC,res.tab,tmp.tab,1.0,0.0);
		d_s0s1_1d_sym8(m2[i].dimension,2,tmp.tailles_s1,PERIODIC,tmp.tab,res.tab,1.0,0.0); */
		d_s0s1_1d_sym8(m2[i].dimension, 0, m1[i].tailles_s1, PERIODIC, m2[i].tab, res[i].tab, 1.0, 0.0);
		d_s0s1_1d_sym8(m2[i].dimension, 1, m1[i].tailles_s1, PERIODIC, res[i].tab, tmp[i].tab, 1.0, 0.0);
		d_s0s1_1d_sym8(m2[i].dimension, 2, m1[i].tailles_s1, PERIODIC, tmp[i].tab, res[i].tab, 1.0, 0.0);

		//printDiffMatrice(&m1[i],&res[i]);
		if(compareMatrice(&m1[i],&res[i],1.0e-7)){
			printf("success ! \n");
		}
	}
}
